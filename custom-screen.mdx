...
<Warning>
  Native iOS in `flowboard-pckg-swift` `1.0.0` does not yet expose a `customScreenBuilder` or an in-flow `type: "custom"` screen replacement API. If you build with Swift today, the closest workaround is to intercept a custom action with `onCustomAction(...)` and present your own native screen, sheet, or paywall from there.
</Warning>
